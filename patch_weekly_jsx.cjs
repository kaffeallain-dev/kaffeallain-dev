...
const oldJsxRegex = /\{\/\* Nutrition Gaps & Trends \*\/\}[\s\S]*?(?=\s*<\/div>\s*<\/div>\s*\}\)\s*<\/div>\s*\);\s*\})/m;
...
if (!oldJsxRegex.test(code)) {
  console.log("Could not find old JSX to replace.");
} else {
  code = code.replace(oldJsxRegex, newJsx);
  fs.writeFileSync('src/views/AnalyticsView.tsx', code);
  console.log("Patched weekly JSX successfully.");
}
