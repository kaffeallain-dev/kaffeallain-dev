...
const oldNextMealFn = `  // Determine next best meal
  const getNextBestMeal = () => {
    const goal = settings.profile?.primaryGoal || '';
    if (gap.includes('Vegetables')) return commonFoods.find(f => f.name.toLowerCase().includes('eru') || f.name.toLowerCase().includes('ndolé')) || commonFoods[0];
    if (goal === 'Build Muscle') return commonFoods.find(f => f.protein > 20) || commonFoods[0];
    if (goal === 'Lose Weight') return commonFoods.find(f => f.calories < 400 && (f.fiber && f.fiber > 2)) || commonFoods[0];
    return commonFoods.find(f => f.category === 'Traditional Dishes') || commonFoods[0];
  };
  const nextMeal = getNextBestMeal();
  const nextMealAnalysis = nextMeal ? RecommendationEngine.analyzeFood(nextMeal) : null;`;

const newNextMealFn = `  // Determine next best meal
  const recommendation = RecommendationEngine.getContextualRecommendation(
    settings.profile,
    todayLogs,
    weeklyLogs,
    commonFoods // Using commonFoods as available candidates
  );
  const nextMeal = recommendation.food;
  const nextMealAnalysis = nextMeal ? RecommendationEngine.analyzeFood(nextMeal) : null;
  const contextualReason = recommendation.reason;`;

code = code.replace(oldNextMealFn, newNextMealFn);

// Now update the UI to display contextual reason
...
if (code.includes(oldNextMealFn)) {
  code = code.replace(oldNextMealFn, newNextMealFn);
  ...
  console.log("oldNextMealFn not found.");
}
